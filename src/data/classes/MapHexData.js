...
export class MapHexData {
  ...
  isValid(){
    ...
    if( !!this.location && ! FindResourceById(GameResources.Location, this.location.id)) return false;
    if( !!this.rampagingEnemyType && !! this.rampagingEnemy ) return false;
    if( !!this.rampagingEnemyType && ! FindEnumByValue(GameValues.EnemyType, this.rampagingEnemyType.value)) return false;
    // TODO(ylaunay) support loading local mods GUIDs
    if( !!this.rampagingEnemy && ! FindResourceById(GameResources.Enemy, this.rampagingEnemy.id)) return false;
  ...
  static FromJson(json){
    ...
    data.rampagingEnemy = !! json.rampagingEnemy ? FindResourceById(GameResources.Enemy, json.rampagingEnemy) : null;
    data.rampagingEnemyType = !! json.rampagingEnemyType ? FindEnumByValue(GameValues.EnemyType, json.rampagingEnemyType) : null;
