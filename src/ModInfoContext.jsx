...
class ModuleDescriptor {
  ...
  static async Load(modInfo, modId, modType, dataParserFn){
    const modJson = await getTextContentOfFile(
      modInfo.getModuleDescriptorPath(modId),
      modInfo.zip,
    );
...
async function loadModules(modInfo, modulesList, moduleType, modParserFn){
  ...
  for( const modId of modulesList){
    ...
    if( module.guid !== modId ){
      // TODO(ylaunay) show error on UI
      console.log(`Mod '${modId}' folder name should be the same as the module id '${module.guid}'`);
      continue;
    }
...
export function ModInfoProvider({ children }) {
  ...
  const loadModFromZipFile = async (zipFile) => {
    ...
    if( modInfo == null ){
      ...
      // Load all modules
      const modules = {};     
      modules.rewardsMods = loadModules(modInfo, modInfo.descriptor.rewardsMods, RewardData.constructor.name, (json) => {
      ...
      // });
      modules.enemyMods = loadModules(modInfo, modInfo.descriptor.enemyMods, EnemyData.constructor.name, (json) => {
        return EnemyData.FromJson(json);
      });
      modules.terrainMods = loadModules(modInfo, modInfo.descriptor.terrainMods, TerrainData.constructor.name, (json) => {
        return TerrainData.FromJson(json);
      });
      modules.mapSectionMods = loadModules(modInfo, modInfo.descriptor.mapSectionMods, MapSectionData.constructor.name, (json) => {
